...
Or install gh for one-command sharing:
  ...
  /hub share `+t+" "+n,"info");return}let E=a.username;e.ui.notify(`\u{1F4E4} Sharing as ${E}...`,"info");try{let s=`/tmp/soma-share-${n}-${Date.now()}`;w(`gh repo clone ${q} "${s}" -- --depth 1`,{encoding:"utf-8",stdio:"pipe"});let f=`${E}/${t}-${n}`;w(`git checkout -b "${f}"`,{cwd:s,encoding:"utf-8",stdio:"pipe"});let b=F(t,n,u,I);if(t==="script"){let C=h(s,"scripts",n);T(C,{recursive:!0}),j(h(C,`${n}.sh`),I,"utf-8");try{N(h(C,`${n}.sh`),493)}catch{}j(h(C,"README.md"),b.draft,"utf-8")}else{let C=A(t);j(h(s,C,`${n}.md`),I,"utf-8")}if(b.issues.length>0){let C=_(b);e.ui.notify(`\u{1F4CB} Pre-submit quality check (${b.quality}%):
