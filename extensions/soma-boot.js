...
`).slice(0,15);for(let v of C)if(!v.startsWith("#!")){if(i.description==="\u2014"&&v.startsWith("# ")){let I=v.replace(/^#\s*/,"");I=I.replace(/^\S+\.\w+\s*[—–-]\s*/,""),I.length>0&&(i.description=I);continue}if(/^#\s*USE WHEN:/i.test(v)){i.useWhen=v.replace(/^#\s*USE WHEN:\s*/i,"").trim();continue}if(/^#\s*Related muscles?:/i.test(v)){let _=v.replace(/^#\s*Related muscles?:\s*/i,"").split(",").map(R=>R.trim().replace(/\s*\(.*\)/,"").replace(/\s*$/,"")).filter(R=>R.length>0);i.relatedMuscles.push(..._);continue}if(i.description==="\u2014"&&v.startsWith("// ")&&!v.startsWith("// @")){let I=v.replace(/^\/\/\s*/,"");I=I.replace(/^\S+\.\w+\s*[—–-]\s*/,""),I.length>0&&(i.description=I)}}}catch{}return i}S(St,"getScriptMeta");function Be(){let h=process.env.HOME||process.env.USERPROFILE||"";return M(h,".pi","agent","sessions")}S(Be,"getAgentSessionDir");function Ee(h){let o=`--${h.replace(/^[/\\]/,"").replace(/[/\\:]/g,"-")}--`;return M(Be(),o)}S(Ee,"encodeCwdToSessionDir");function Fe(h=10,o,i){if(h<=0)return[];let y=i||process.cwd(),C=[];C.push(Ee(y));let v=be(y);for(;v!==y&&v!=="/"&&v!==".";){C.push(Ee(v));let P=be(v);if(P===v)break;v=P}let I=Be();if(O(I))try{let P=ie(I);for(let E of P){let Y=M(I,E);!E.startsWith("--")&&Se(Y).isDirectory()&&C.push(Y)}}catch{}let _=[],R=new Set;for(let P of C)if(!(!O(P)||R.has(P))){R.add(P);try{let E=ie(P).filter(Y=>Y.endsWith(".jsonl"));for(let Y of E){let J=M(P,Y);if(!(o&&J===o))try{let Z=Se(J);_.push({path:J,mtime:Z.mtimeMs})}catch{}}}catch{}}_.sort((P,E)=>E.mtime-P.mtime);let q=[];for(let P of _){if(q.length>=h)break;try{let Y=X(P.path,"utf-8").trim().split(`
`),J=[];if(Y.length>0)try{let K=JSON.parse(Y[0]);if(K.type==="session"&&K.cwd){let F=K.cwd,T=(i||process.cwd()).replace(/\/$/,"");if(!T.startsWith(F)&&!F.startsWith(T))continue}}catch{}for(let K of Y)try{let F=JSON.parse(K);if(F.type!=="message")continue;let T=F.message;if(!T||!T.role||!["user","assistant"].includes(T.role))continue;let D="";if(Array.isArray(T.content)){for(let x of T.content)if(x.type==="text"&&x.text){D=x.text;break}}else typeof T.content=="string"&&(D=T.content);if(!D||D.startsWith("[cache keepalive")||D.startsWith("[Soma Boot")||D.startsWith("[Soma ")||D.includes("FLUSH COMPLETE")||D.includes("BREATHE COMPLETE"))continue;J.push({role:T.role,text:D,timestamp:F.timestamp})}catch{}let Z=h-q.length,Q=J.slice(-Z);q.unshift(...Q)}catch{}break}return q.slice(-h)}S(Fe,"scanSessionLogs");function Le(h){if(h.length===0)return"";let o=[];for(let i of h){let y=i.role==="user"?"**User:**":"**Assistant:**",C=i.role==="assistant"?500:300,v=i.text;v.length>C&&(v=v.slice(0,C)+"\u2026"),o.push(`${y} ${v}`)}return`---
...
**Available automations (not loaded):** ${l}`)}}break}case"scripts":{let s=[B(o.path,"scripts",i)];if(i.inherit.tools&&n.length>1)for(let r=1;r<n.length;r++)s.push(B(n[r].path,"scripts",i));let d=new Set,l=[];for(let r of s)if(O(r))try{let u=i?.scripts?.extensions??bt,a=S(($,g)=>{if(!(g>2))try{let p=ie($,{withFileTypes:!0});for(let f of p)if(!(f.name.startsWith("_")||f.name.startsWith("."))){if(f.isDirectory())a(M($,f.name),g+1);else if(f.isFile()&&u.some(k=>f.name.endsWith(k))&&!d.has(f.name)){d.add(f.name);let k=M($,f.name),A=St(k,f.name);l.push({name:f.name,dir:$,meta:A})}}}catch{}},"scanScriptDir");a(r,0)}catch{}if(l.length>0){let r=M(o.path,"state.json"),u={};try{u=JSON.parse(X(r,"utf-8")).scripts??{}}catch{}l.sort((g,p)=>{let f=u[g.name]?.count??0,k=u[p.name]?.count??0;return k!==f?k-f:g.name.localeCompare(p.name)});let a=[`## Available Scripts
`,`**Before coding, check if a script already handles the task. Read the associated muscle first.**
`,"| Script | What it does | Uses |","|--------|-------------|------|",...l.map(({name:g,dir:p,meta:f})=>{let k=u[g]?.count??0,A=k>0?`${k}`:"";return`| \`${g}\` | ${f.description} | ${A} |`}),"","Run with `bash <path>`. Use `--help` for options.",""],$=new Set;for(let{meta:g}of l)for(let p of g.relatedMuscles)$.add(p);if($.size>0&&P.length>0){let g=[];for(let p of $){let f=P.find(k=>k.name===p);if(f?.digest){let k=l.filter(A=>A.meta.relatedMuscles.includes(p)).map(A=>A.name.replace(/\.sh$/,""));g.push(`**${p}** (used by: ${k.join(", ")}): ${f.digest.trim()}`)}}g.length>0&&a.push(`
...
Session ID: \`${L}\``:"";y.boot(`resume: ${w.length} changes since last boot`),e.hasUI&&h.sendUserMessage(`[Soma Boot \u2014 resumed, delta only]

**Changes since last boot:**
${w.join(`
`)}

Everything else unchanged. System prompt is current.${H}`,{deliverAs:"followUp"})}return}y.boot("resume: no fingerprint \u2014 minimal boot (system prompt is current)"),Q=!0;let $=L?`
Session ID: \`${L}\``:"",g=o?M(B(o.path,"sessions",i),ae()):null,p=o?M(B(o.path,"preloads",i),re()):null,f=g||p?`

Session files:
${g?`- Session log: \`${g}\`
`:""}${p?`- Preload: \`${p}\`
`:""}`:"",k=s.filter(b=>b.includes(".soma Changes")||b.includes("Recent Changes")),A=k.length>0?`

${k.join(`
`)}`:"";h.appendEntry("soma-boot",{timestamp:Date.now(),resumed:!0,fingerprint:r}),e.hasUI&&h.sendUserMessage(`[Soma Boot \u2014 resumed]

Identity, protocols, and muscles are in your system prompt. Continue where you left off.${A}${$}${f}`,{deliverAs:"followUp"});return}if(!c&&o){let a=M(o.path,".boot-target");if(O(a))try{let $=X(a,"utf-8"),g=JSON.parse($);if(Ie(a),g.type==="map"&&g.name){let p=Ne(g.name,o,i);if(p){y.boot(`MAP loaded: ${p.name} (status: ${p.status})`),Oe(p),p.promptConfig&&(D=p.promptConfig,y.boot(`plan overrides active: ${Object.keys(p.promptConfig).join(", ")}`));let f=$t(g.name,o,i);f&&(s.unshift(`
---
...
`)}if(g.type==="focus"&&g.keyword){y.boot(`focus mode: "${g.keyword}"`);let p=g.promptConfig??{};if(P&&P.length>0){let b=yt(P,g.keyword);if(b.length>0){let w=p.heatOverrides?.muscles??{},N=p.forceInclude?.muscles??[];for(let{muscle:U,score:j}of b){let G=w[U.name]??0;j>=8&&!N.includes(U.name)&&N.push(U.name),j+2>G&&(w[U.name]=j+2)}p.heatOverrides={...p.heatOverrides,muscles:w},p.forceInclude={...p.forceInclude,muscles:N},y.boot(`focus: ${b.length} muscles matched via triggers/tags/keywords`)}}if(D=p,p&&y.boot(`focus overrides: ${Object.keys(p).join(", ")}`),g.preloadPath){let b=null;try{let w=M(o.path,g.preloadPath);O(w)&&(b=X(w,"utf-8"))}catch{}b&&(s.unshift(`
---
## Focus Context: ${g.keyword}

${b}
`),y.boot(`focus preload injected: ${g.preloadPath}`))}let f=g.relatedMaps??[],k=f.slice(0,3);for(let b of k){let w=Ne(b,o,i);if(w){let N=ne(w.content);if(s.push(`
---
## Related MAP: ${w.name}

${N}
`),y.boot(`focus MAP loaded: ${w.name}`),w.promptConfig){let U=w.promptConfig;if(U.heatOverrides?.muscles){let j=p.heatOverrides?.muscles??{};for(let[G,V]of Object.entries(U.heatOverrides.muscles))V>(j[G]??0)&&(j[G]=V);p.heatOverrides={...p.heatOverrides,muscles:j}}if(U.heatOverrides?.protocols){let j=p.heatOverrides?.protocols??{};for(let[G,V]of Object.entries(U.heatOverrides.protocols))V>(j[G]??0)&&(j[G]=V);p.heatOverrides={...p.heatOverrides,protocols:j}}if(U.forceInclude?.muscles){let j=p.forceInclude?.muscles??[];for(let G of U.forceInclude.muscles)j.includes(G)||j.push(G);p.forceInclude={...p.forceInclude,muscles:j}}U.supplementaryIdentity&&!p.supplementaryIdentity?.includes(U.supplementaryIdentity)&&(p.supplementaryIdentity=(p.supplementaryIdentity??"")+`
`+U.supplementaryIdentity),y.boot(`focus: merged MAP ${w.name} prompt-config`),D=p}Oe(w)}}f.length>3&&s.push(`
...
`)}}catch($){y.boot(`boot-target read failed: ${$}`);try{Ie(a)}catch{}}}let u=process.env.SOMA_INHALE==="1";if(!c&&o&&u){let a=te(o,i.preload.staleAfterHours,i);if(a&&!a.stale){let p=a.stale?" \u26A0\uFE0Fstale":"";s.unshift(`
---
## Preload (from last session${p})

${a.content}
`),y.boot(`preload auto-injected: ${a.name} (${Math.floor(a.ageHours)}h old)`)}let $=Math.min(i.preload.lastSessionLogs??0,5);if($>0&&o)try{let p=B(o.path,"sessions",i);if(O(p)){let f=ie(p).filter(k=>k.endsWith(".md")&&!k.startsWith(".")&&!k.startsWith("_")).sort().reverse().slice(0,$);if(f.length>0){let k=f.map(A=>{let b=X(M(p,A),"utf-8").trim(),w=3e3,N=b.length>w?b.slice(0,w)+`

[... truncated \u2014 full log: memory/sessions/${A}]`:b;return`### ${A}
...
Session ID: \`${L}\``:"",$=o?M(B(o.path,"sessions",i),ae()):null,g=o?M(B(o.path,"preloads",i),re()):null,p=$||g?`
...
`+(v?"```\n"+v.slice(0,2e3)+"\n```":"No identity found in chain."),{deliverAs:"followUp"});return}let f=[];f.push(`**Compiled System Prompt** \u2014 ${l.block.length} chars (~${l.estimatedTokens} tokens)`),f.push(`Full replacement: ${l.fullReplacement} | Cached: ${F}`),f.push(""),f.push("**Sections:**");for(let[b,w]of r){let N=l.block.includes(w);f.push(`  ${N?"\u2705":"\u274C"} ${b}`)}f.push(""),f.push(`**Identity:** ${v?`${v.length} chars`:"\u274C NONE"} \u2192 ${l.block.includes("# Identity")?"in prompt \u2705":"MISSING from prompt \u274C"}`),f.push(`  persona.name: ${i.persona?.name??"(null)"} | emoji: ${i.persona?.emoji??"(null)"}`),f.push(""),f.push(`**Protocols:** ${l.protocolCount} in prompt (max ${i.protocols?.maxBreadcrumbsInPrompt??12})`);for(let b of u){let w=b.heat>=$?"\u{1F534}":b.heat>=a?"\u{1F7E1}":"\u26AA",N=b.heat>=a?"\u2705":"\u2014";f.push(`  ${w} ${b.name}: heat=${b.heat} ${N}`)}f.push(""),f.push(`**Muscles:** ${l.muscleCount} digests in prompt (max ${i.muscles?.maxDigest??5})`);for(let b of P.slice(0,10)){let w=b.heat>=p?"\u{1F534}":b.heat>=g?"\u{1F7E1}":"\u26AA",N=b.status!=="active"?` [${b.status}]`:"";f.push(`  ${w} ${b.name}: heat=${b.heat}${N}`)}f.push("");let k=e.getContextUsage?.();f.push("**Runtime:**"),f.push(`  Context: ${k?.percent!=null?Math.round(k.percent)+"%":"unknown"}`);let A=z()?.get("breathe:state")?.()??{};f.push(`  Warnings sent at: ${A.pct>0?Math.round(A.pct)+"%":"none yet"}`),f.push(`  Thresholds: ${JSON.stringify(i.context??{notifyAt:50,warnAt:70,urgentAt:80,autoExhaleAt:85})}`),f.push(""),f.push("\u{1F4A1} `/soma prompt full` \u2014 dump compiled prompt | `/soma prompt identity` \u2014 identity debug"),e.ui.notify(f.join(`
...
${r}`,"info")},"deliver");if(c[0]==="tools"&&c.length>=2){if(!o){e.ui.notify("No .soma/ found.","error");return}let r=M(o.path,"amps","scripts","soma-stats.sh");if(!O(r)){e.ui.notify("soma-stats.sh not found.","error");return}let u=c.slice(1).join(" "),a=`bash "${r}" tools ${u} --cwd "${process.cwd()}"`;if(m)try{let $=W(a,{encoding:"utf-8",timeout:15e3,stdio:["pipe","pipe","pipe"]}).trim();s($||"No matching tool calls found.")}catch($){s($.stdout?.trim()||"No matching tool calls found.")}else e.ui.notify(`\u{1F50D} Searching tool calls: \`soma-stats.sh tools ${u}\``,"info");return}let d=10;for(let r of c)/^\d+$/.test(r)&&(d=parseInt(r,10));let l=e.sessionManager.getSessionFile?.()||void 0;try{let r=Fe(d,l),u="";if(r.length>0?u+=Le(r):u+=`No recent conversation logs found.
`,o){let a=M(o.path,"amps","scripts","soma-stats.sh");if(O(a))try{let $=W(`bash "${a}" --cwd "${process.cwd()}"`,{encoding:"utf-8",timeout:5e3,stdio:["pipe","pipe","pipe"]}).trim();u+=`
${$}
